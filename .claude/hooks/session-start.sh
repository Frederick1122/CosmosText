...
# чтобы в облачной сессии работали validate_content.py и смоук-тест.
set -euo pipefail
...
if [ ! -x "$GODOT_BIN" ]; then
	...
	python3 -m zipfile -e "$tmp_zip" "$GODOT_DIR"
	rm -f "$tmp_zip"
	chmod +x "$GODOT_BIN"
fi
...
# Импорт ресурсов (.godot/), иначе первый headless-запуск сцены может падать.
